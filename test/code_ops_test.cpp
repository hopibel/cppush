...
TEST_CASE("Instruction: code_append") {
	...
	SECTION("Both atoms"){
		...
		env.get_stack<cppush::Code_ptr>().insert(env.get_stack<cppush::Code_ptr>().end(), {second, first});
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(env.get_stack<cppush::Code_ptr>().back()->is_list());
		auto& stack = env.get_stack<cppush::Code_ptr>().back()->get_stack();
	...
	SECTION("Atom and list"){
		...
		SECTION("Append list to atom") {
			env.get_stack<cppush::Code_ptr>().insert(env.get_stack<cppush::Code_ptr>().end(), {atom, code_list});
			op(env);
			REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
			REQUIRE(env.get_stack<cppush::Code_ptr>().back()->is_list());
			auto& stack = env.get_stack<cppush::Code_ptr>().back()->get_stack();
		...
		SECTION("Append atom to list") {
			env.get_stack<cppush::Code_ptr>().insert(env.get_stack<cppush::Code_ptr>().end(), {code_list, atom});
			op(env);
			REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
			REQUIRE(env.get_stack<cppush::Code_ptr>().back()->is_list());
			auto& stack = env.get_stack<cppush::Code_ptr>().back()->get_stack();
...
TEST_CASE("Instruction: code_atom") {
	...
	SECTION("List") {
		auto code_list = std::make_shared<cppush::CodeList>();
		env.push<cppush::Code_ptr>(code_list);
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().empty());
...
TEST_CASE("Instruction: code_car") {
	...
	SECTION("Atom") {
		...
		env.push<cppush::Code_ptr>(atom);
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>() == std::vector<cppush::Code_ptr>{atom});
	}

	SECTION("List") {
		auto atom_a = std::make_shared<cppush::Literal<int>>(0);
		auto atom_b = std::make_shared<cppush::Literal<int>>(1);
		auto list_ab = std::vector<cppush::Code_ptr>{atom_a, atom_b};
		auto code_list = std::make_shared<cppush::CodeList>(list_ab);
		env.push<cppush::Code_ptr>(code_list);
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>() == std::vector<cppush::Code_ptr>{atom_a});
	}
}
...
TEST_CASE("Instruction: code_cdr") {
	...
	SECTION("Atom") {
		...
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == cppush::CodeList());
	}
	...
	SECTION("List") {
		...
		auto atom_b = std::make_shared<cppush::Literal<int>>(1);
		auto list_ab = std::vector<cppush::Code_ptr>{atom_a, atom_b};
		auto list_b = std::vector<cppush::Code_ptr>{atom_b};
		auto code_list = std::make_shared<cppush::CodeList>(list_ab);
		env.push<cppush::Code_ptr>(code_list);
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == cppush::CodeList(list_b));
	}
}
...
TEST_CASE("Instruction: code_cons") {
	...
	SECTION("First item is an atom") {
		...
		env.push<cppush::Code_ptr>(second);
		env.push<cppush::Code_ptr>(first);
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		auto& code_list = env.get_stack<cppush::Code_ptr>().back()->get_stack();
	...
	SECTION("First item is a list") {
		auto first = std::make_shared<cppush::CodeList>(
				std::vector<cppush::Code_ptr>{std::make_shared<cppush::Literal<int>>(0)}
				);
		env.push<cppush::Code_ptr>(second);
		env.push<cppush::Code_ptr>(first);
		op(env);
...
TEST_CASE("Instruction: code_container") {
	...
	*/
	...
	std::vector<cppush::Code_ptr> list_a{a};
	std::vector<cppush::Code_ptr> list_ca{c, std::make_shared<cppush::CodeList>(list_a)};
	std::vector<cppush::Code_ptr> list_bca{b, std::make_shared<cppush::CodeList>(list_ca)};
	std::vector<cppush::Code_ptr> list_da{d, std::make_shared<cppush::CodeList>(list_a)};

	std::vector<cppush::Code_ptr> list_full{
	...
	SECTION("Find container") {
		env.push<cppush::Code_ptr>(std::make_shared<cppush::CodeList>(list_a));
		env.push<cppush::Code_ptr>(container);
		op(env);

		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == cppush::CodeList(list_ca));
	}
...
TEST_CASE("Instruction: code_do") {
	...
	std::shared_ptr<cppush::Instruction> pop_insn = std::make_shared<cppush::Instruction>(
			cppush::protected_pop<cppush::Code_ptr>, "code_pop");
	std::shared_ptr<cppush::CodeList> code_list = std::make_shared<cppush::CodeList>();

	env.push<cppush::Code_ptr>(code_list);
	op(env);
	REQUIRE(env.get_stack<cppush::Code_ptr>() == std::vector<cppush::Code_ptr>{code_list});
...
TEST_CASE("Instruction: code_do*range") {
	...
	env.push<cppush::Code_ptr>(std::make_shared<cppush::CodeList>(body));
...
TEST_CASE("Instruction: code_do*count") {
	...
	SECTION("count < 1") {
		...
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == body);
	}
...
TEST_CASE("Instruction: code_extract") {
	...
	*/
	...
	std::vector<cppush::Code_ptr> indexed_list{
	...
	SECTION("Positive indexes") {
		for (int i = 0; i < index_upper; ++i) {
			env.get_stack<cppush::Code_ptr>().clear();
			...
			REQUIRE(env.get_stack<int>().empty());
			REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
			REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *indexed_list[i]);
		}
	}
	...
	SECTION("Negative indexes") {
		for (int i = -1; i > -index_upper; --i) {
			...
			REQUIRE(env.get_stack<int>().empty());
			REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
			REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *indexed_list[std::abs(i)]);
		}
	}
...
TEST_CASE("Instruction: code_from_bool") {
	...
	op(env);
	REQUIRE(env.get_stack<bool>().empty());
	REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *bool_literal);
}
...
TEST_CASE("Instruction: code_from_float") {
	...
	REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *float_literal);
}
...
TEST_CASE("Instruction: code_from_int") {
	...
	op(env);
	REQUIRE(env.get_stack<int>().empty());
	REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *int_literal);
}
...
TEST_CASE("Instruction: code_if") {
	...
	env.push<cppush::Code_ptr>(std::make_shared<cppush::CodeList>(branchB));
	env.push<cppush::Code_ptr>(std::make_shared<cppush::CodeList>(branchA));
...
TEST_CASE("Instruction: code_insert") {
	...
	*/
	...
	SECTION("Positive index") {
		env.push<cppush::Code_ptr>(d);
		env.push<cppush::Code_ptr>(container);
		env.push<int>(4);
		op(env);
		REQUIRE(env.get_stack<int>().empty());
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		auto last = env.get_stack<cppush::Code_ptr>()[0]->get_stack().back()->get_stack()[0];
		REQUIRE(*last == *d);
	}
...
TEST_CASE("Instruction: code_list") {
	...
	*/
	...
	op(env);
	REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
	auto& stack = env.get_stack<cppush::Code_ptr>()[0]->get_stack();
	REQUIRE(*stack[0] == *b);
	REQUIRE(*stack[1] == *a);
}
...
TEST_CASE("Instruction: code_member") {
	...
	*/
	...
	auto ab = std::make_shared<cppush::CodeList>(std::vector<cppush::Code_ptr>{a, b});

	SECTION("( A B ) A") {
		env.push<cppush::Code_ptr>(a);
		env.push<cppush::Code_ptr>(ab);
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().empty());
		REQUIRE(env.pop<bool>() == true);
		REQUIRE(env.get_stack<bool>().empty());
...
TEST_CASE("Instruction: code_nth") {
	...
	*/
	...
	SECTION("Empty list") {
		...
		env.push<cppush::Code_ptr>(std::make_shared<cppush::CodeList>(empty_list));
		env.push<int>(0);
		op(env);

		REQUIRE(env.get_stack<int>().empty());
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == empty_list);
	}
	...
	SECTION("Atom") {
		...
		REQUIRE(env.get_stack<int>().empty());
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *a);
	}
	...
	SECTION("Nth") {
		...
		REQUIRE(env.get_stack<int>().empty());
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *list_a_ptr);
	}
...
TEST_CASE("Instruction: code_size") {
	...
	auto nil = std::make_shared<cppush::CodeList>();
	auto a = std::make_shared<cppush::Literal<int>>(1);
	auto aa = std::make_shared<cppush::CodeList>(std::vector<cppush::Code_ptr>{a, a});
	auto nested_aa = std::make_shared<cppush::CodeList>(std::vector<cppush::Code_ptr>{a, aa});
	...
	SECTION("( A ( A A ) ) = 5") {
		env.push<cppush::Code_ptr>(nested_aa);
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().empty());
...
TEST_CASE("Instruction: code_subst") {
	...
	auto c = std::make_shared<cppush::Literal<int>>(3);
	auto ab = std::make_shared<cppush::CodeList>(std::vector<cppush::Code_ptr>{a, b});
	auto ac = std::make_shared<cppush::CodeList>(std::vector<cppush::Code_ptr>{a, c});
	...
	SECTION("A B A = B") {
		...
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *b);
	}
	...
	SECTION("( A B ) C B = ( A C )") {
		...
		op(env);
		REQUIRE(env.get_stack<cppush::Code_ptr>().size() == 1);
		REQUIRE(*env.get_stack<cppush::Code_ptr>().back() == *ac);
	}
}
